...
into the locations given by argument(s). Locations pointed by each
argument are filled with their corresponding type of value
requested in the format string. There must be the same number of
type specifiers in format string than arguments passed.
...
string. There must be the same number of these parameters than the number of format tags. NOTE: These arguments must be pointers: if you want to store the result of a +scanf+ operation on a standard variable you should precede it with the reference operator, i.e. an ampersand sign (`&`), like in: `sscanf (buffer,"%d",&n);`
