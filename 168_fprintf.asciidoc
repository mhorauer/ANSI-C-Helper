...
[source,c]
---------------------
include::src/fprintf.c[]
---------------------

This example prompts 3 times the user for a
name and then writes them to given file each one in a line with a fixed
length (a total of 19 characters + newline). Two format tags are used: `%d` (Signed decimal integer) and `%-10.10s` (left aligned `-`, minimum of 10 characters `10`, maximum of 10 characters `.10`, String `s`).
