...
The +vfprintf+ function formats a series of strings and
numeric values and builds a string to write to the output stream
using the +fputc+ function. The function is similar to the
counterpart +fprintf+, but it accepts a pointer to a list of
arguments instead of an argument list.

The +fmtstr+ argument is a pointer to a format string and has the
same form and function as the +fmtstr+ argument for the +printf+
function. Refer to +fprintf+ for a description of the format string.
The +va_list+ points to a list of arguments that are converted and
output according to the corresponding format specifications in the
format.
